...
/* ******************************************************************************** */
/* Common result structure for ring */
struct cmd_ring_result {
	cmdline_fixed_string_t cmd_ring;
	cmdline_fixed_string_t cmd_op;
	cmdline_fixed_string_t name;
	uint32_t count;
	uint32_t socket;
	uint32_t flags;
...
/* Common CLI fields for ring */
cmdline_parse_token_string_t cmd_ring_ring =
	TOKEN_STRING_INITIALIZER
		(struct cmd_ring_result,
		 cmd_ring, "ring");
cmdline_parse_token_string_t cmd_ring_op =
	TOKEN_STRING_INITIALIZER
		(struct cmd_ring_result,
		 cmd_op, "add#del");
cmdline_parse_token_string_t cmd_ring_name =
	TOKEN_STRING_INITIALIZER
		(struct cmd_ring_result,
		 name, NULL);
cmdline_parse_token_num_t cmd_ring_count =
	TOKEN_NUM_INITIALIZER
		(struct cmd_ring_result,
		 count, UINT32);
cmdline_parse_token_num_t cmd_ring_socket =
	TOKEN_NUM_INITIALIZER
		(struct cmd_ring_result,
		 socket, UINT32);
cmdline_parse_token_num_t cmd_ring_flags =
	TOKEN_NUM_INITIALIZER
		(struct cmd_ring_result,
		 flags, UINT32);
...
cmd_ring_parsed(
	...
	__attribute__((unused)) void *data)
{
	struct cmd_ring_result *res = parsed_result;

	if (!strcmp(res->cmd_op, "add")) {
		struct rte_ring *rq = rte_ring_create(res->name,
			res->count, res->socket, res->flags);
		if (rq == NULL)
			printf("Cannot create rx ring queue %s %d %d %x\n", 
				res->name, res->count, res->socket, res->flags);
	} else if (!strcmp(res->cmd_op, "del")) {
	}
}

cmdline_parse_inst_t cmd_ring = {
	.f = cmd_ring_parsed,
	.data = NULL,
	.help_str = "ring add <name> <count> <socket> <flags>",
	.tokens = {
		(void *)&cmd_ring_ring,
		(void *)&cmd_ring_op,
		(void *)&cmd_ring_name,
		(void *)&cmd_ring_count,
		(void *)&cmd_ring_socket,
		(void *)&cmd_ring_flags,
		NULL,
	},
};
